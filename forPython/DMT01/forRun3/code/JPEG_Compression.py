...
def rgb2ycbcr(image):
    ...
    ycbcr = image.dot(args.T)  # 矩阵转置后向量相乘
    ycbcr[:, :, [0]] += 16
    ycbcr[:, :, [1, 2]] += 128
...
def padding(image):
    ...
    new_image[:row, :col] = image  # 将 image 数组的值复制到这个选定的区域。剩余右下角区域为0
    return new_image
...
def sampling(image, row, col):
    ...
    for i in range(row):
        for j in range(col):
            samp_y.append(image[i, j, 0]) #Y不变
            if i % 2 == 0 and j % 2 == 0:
                samp_cr.append(image[i, j, 2])  #Cb信号采样
            elif j % 2 == 0:
                samp_cb.append(image[i, j, 1])  #Cr信号隔行采样

    samp_y = np.array(samp_y).reshape((row, col))
    samp_cb = np.array(samp_cb).reshape((row // 2, col // 2)) #取四分之一
...
def split_blocks(image, block_size=8):
    ...
    for block in row_blocks:
        result.append(np.split(block, image.shape[1] / block_size, axis=1)) #axis = 1 纵向切分  axis = 0 列
...
if __name__ == '__main__':
    ...
    print("new row and col is", row_new, col_new)   #转成了16的整数倍
